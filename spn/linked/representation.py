        ...
        while nodes_to_process:
            ...
            if isinstance(curr_node, SumNode):
                ...
                for i, child in enumerate(curr_node.children):
                    # path.add(child)
                    child_output = curr_node.log_weights[i] + child.log_val
...
def extract_features_nodes_mpn(mpn,
    ...
    # # save scopes and other info?
    # if output_feature_info is not None:
    #     save_feature_info(mpn, nodes_id_assoc, output_feature_info)
    ...
    while nodes_to_process:
        ...
        if isinstance(curr_node, SumNode):
            ...
            if present and feature_id < n_features:
                ...
                if not binary:
                    max_child = curr_node.children[embedding[feature_id]]
                elif latent_var_sizes:
                    raise NotImplementedError('Still to implement the binary case')
                else:
                    raise ValueError(
                        'Binary latent array specified but no latent RV size vecotr given')
...
def extract_features_nodes_by_scope(spn,
    ...
    if output_feature_info is not None:
        logging.error('Cannot save feature file for scopewise embeddings')
    ...
    for i in range(n_instances):
        ...
        # #
        # # normalizing it by the number of nodes
        # if normalizing:
        #     # if i == 0:
        #     #     print('before norm', repr_data[i])
        #     repr_data[i] /= scope_num_nodes
        #     # if i == 0:
        #     #     print('after norm', repr_data[i])
